...
public class UpdateDataView {
    ...
    int length=0;  Label labelh3date; Label labelh2date; Label labelh1date;List<EmployeeData> employeeList;
	 BorderPane createTab(RepositoryManager repositoryManager1, List<EmployeeData> employeeList1, List<EmployeeInsertedData> dataLis) {
		 ...
		 Dimension screenSize = new Dimension(1300,650);
	        ...
	        centerStackPane.setPrefWidth(0.86 *screenSize.getWidth()-70); // 40% of the total width
	        	...
	        	dataLisfixday=dataLisfixday(LocalDate.now());
	        	 for(int i=0;i<=employeeList.size();i++)
	  	       {
	  	    	   ...
	  	    	   else
	  	    	   {
					   ...
					   System.out.println(dataLisfixday.get(i-1));
							 ...
							 for(int i=0;i<length;i++)
							 {
								 ...
								 try {
									 UpdateDataController.updateData(repositoryManager,employeeList.get(i).getIdNumber(), employeeList.get(i).getName(),
											 ...
											 ,LocalDateTime.now()
											 , Integer.toString(entryDate.getMonthValue()),Integer.toString(entryDate.getYear()),outtimehour[i].getText()+"."+outtimeminute[i].getText(),entryDate.toString());
