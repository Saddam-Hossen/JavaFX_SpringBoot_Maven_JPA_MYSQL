            ...
            tabPane.getSelectionModel().selectedItemProperty().addListener((observable, oldTab, newTab) -> {
                ...
                switch (selectedIndex) {
                    case 0:
                    	...
                    	updatePanel(tabContentsummry,new AddDataView().createTab(repositoryManager,employeeList));
                        break;
                     case 1:
                    	
                     	//updatePanel(panel2,new AddEmployeeView().createTab(employeeList));
                    	// updatePanel(tabContentataglance,new UpdateDataView().createTab(employeeList,dataList));
                        // Handle the content update for the second tab
                         updatePanel(tabContentataglance,new UpdateDataView().createTab(repositoryManager,employeeList,dataList));
                        break;
