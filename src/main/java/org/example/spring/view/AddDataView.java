						 ...
						 for(int i=0;i<length;i++)
						 {
							 ...
							 try {
								 new AddDataController().insertData(repositoryManager,employeeList.get(i).getIdNumber(), employeeList.get(i).getName(),
										 ...
										 , Integer.toString(entryDate.getMonthValue()),Integer.toString(entryDate.getYear()),outtimehour[i].getText().trim()+"."+outtimeminute[i].getText().trim(),entryDate.toString());
